...
RuleHelper.prototype = {
    ...
    isAllowedIdentifier(expression, escapeObject, details) {
        ...
        if (this.ruleChecks["variableTracing"] < 1) {
            return false;
        }
    ...
    combineRuleChecks(defaultRuleChecks) {
        ...
        ruleCheckOutput["variableTracing"] = 0;
        if ("variableTracing" in parentRuleChecks) {
            if (parentRuleChecks["variableTracing"] < 2 && parentRuleChecks["variableTracing"] > -1) {
                ruleCheckOutput["variableTracing"] = parentRuleChecks["variableTracing"];
            } else {

                // report() requires an ast node. Providing a nonsense node :(
                this.context.report(
                    {
                        node: {
                            loc: {
                        ...
                        message: `Configuration Error! Setting 'variableTracing' has unexpected value ${parentRuleChecks["variableTracing"]}, expected 0 or 1`
                    });
            }
        }
