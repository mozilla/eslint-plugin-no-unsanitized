    ...
    checkMethod(node) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.ruleChecks, methodName)) {
            ...
            ruleCheck.properties.forEach((propertyId) => {
                const positionedArg = node.arguments[propertyId];
                if (!positionedArg) {
                    // bail out if arguments is supplied as a SpreadElement like `...args`
                    return;
                }
                const shouldCheck = this.shouldCheckMethodCall(node, ruleCheck.objectMatches);
                if (shouldCheck) {
                    const allowedExpression = this.allowedExpression(positionedArg, ruleCheck.escape);
