...
RuleHelper.prototype = {
     ...
     */
    checkProperty(node) {
        const left_property_name = node.left.property.name || node.left.property.value;
        if (Object.prototype.hasOwnProperty.call(this.ruleChecks, left_property_name)) {
            const ruleCheck = this.ruleChecks[left_property_name];
            const details = {};
            if (!this.allowedExpression(node.right, ruleCheck.escape, details)) {
                // Include the additional details if available (e.g. name of a disallowed variable
                // and the position of the expression that made it disallowed).
                if (details.message) {
                    this.context.report(node, `Unsafe assignment to ${left_property_name} (${details.message})`);
                    return;
                }
                this.context.report(node, `Unsafe assignment to ${left_property_name}`);
            }
        }
    },
